...
export class FavouriteReportsComponent implements OnInit {
          ...
          if (res != null && res !== '') {
            ...
            for (const item of arrayRowFilter) {
              ...
              const arrayFilter = this.reportListAll.filter(
                (report: { ReportID: number }) => report.ReportID === itemID
              );
  ...
  filterRow() {
    ...
    if (Array.isArray(selectedMarketList)) {
      selectedMarketList.forEach((market: string) => {
        const arrayFilter = this.reportListAll.filter((report: { Market: {  results: string[] }; }) => report.Market.results.includes(market));
        for (const item of arrayFilter) {
          this.filterredList.push(item); 
        }
      });
    ...
    if (Array.isArray(selectedBusinessUnit)) {
      selectedBusinessUnit.forEach((businessUnit: string) => {
        const arrayFilter = this.reportListAll.filter((report: { BusinessUnit: string; }) => report.BusinessUnit === businessUnit);
        for (const item of arrayFilter) {
          this.filterredList.push(item); 
        }
      });
    ...
    if (Array.isArray(selectedAuthors)) {
      selectedAuthors.forEach((author: string) => {
        const arrayFilter = this.reportListAll.filter((report: { Authors: string; }) => report.Authors === author);
        for (const item of arrayFilter) {
          this.filterredList.push(item); 
        }
      });
    ...
    if (Array.isArray(selectedIndustry)) {
      selectedIndustry.forEach((industry: string) => {
        const arrayFilter = this.reportListAll.filter((report: { Industry: string; }) => report.Industry === industry);
        for (const item of arrayFilter) {
          this.filterredList.push(item); 
        }
      });
  ...
  async getReportsListItems() {
    this.isLoader = true;
    this.apiservice.getAllFavourites(this.loginUserName.toLowerCase()).subscribe((res)=> {
      if (res != null && res !== '') {
  ...
  navigateDetailPage(item: any) {
    this.router.navigate(['/reports/detail-page'], { queryParams: { id: item.ReportID } });
  }
}
